...
def CreateHistory():
    cursor.execute("""
        SELECT Ps.product_name_fr AS search, Pr.product_name_fr AS result
        FROM History
    ...
    while id_selection < len(history):
        ...
        history_liste.append([itera, name_search, name_result])
        id_selection += 1
...
Type 'p' or 'h' and press Enter to validate your choice => ")
    ...
    if user_menu_choice == "h":
        ...
        while run_loop_history_selection == 0:
            try:
                user_history_selection = int(input("Rentrez le nombre associé à votre choix"))
