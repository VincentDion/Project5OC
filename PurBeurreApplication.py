...
while run_main_loop == 0:
    clear()
    print("Bienvenue dans le menu principal de l'application Pur Beurre.\n")
    user_menu_choice = input("\
Pour accéder à la fonctionnalité de remplacement de produit appuyez sur \
la touche [p].\nPour la fonctionnalité d'historique, appuyez sur la touche \
[h]\nVous pouvez également quitter à tout moment l'application en tapant \
[quit] et en validant avec Entrée\n\nAppuyez sur [p] ou [h] et validez \
votre choix en appuyant sur Entrée => ")

    if user_menu_choice == "h":
        HistoryList = clas.History()
        if len(HistoryList.history_list) == 0:
            print("\
\nVotre historique est vide, utilisez l'application de remplacement \
de produit et sauvegardez votre résultat pour remplir l'historique\n")
            run_loop_empty_history = 0
            # Loop to wait user to press Enter and go back to the main menu
            while run_loop_empty_history == 0:
                try:
                    user_history_empty = input("\
Appuyez sur la touche [Entrée] pour retourner au menu principal")
                    assert user_history_empty == "" \
                        or user_history_empty == "quit"
            ...
            while run_loop_history_selection == 0:
                user_history_selection = input("\
Rentrez le nombre associé à votre choix ou tapez [RESET] pour réinitialiser \
l'historique\n(ATTENTION : Réinitialiser l'historique est une commande \
définitive) \n=> ")
                if user_history_selection == "RESET":
                    ...
                    try:
                        ...
                        assert user_history_selection \
                            <= len(HistoryList.history_list)
...
Veuillez rentrer le nombre correspondant")
                        ...
                        print("\
Veuillez rentrer un nombre présent dans cette liste")
                        run_loop_history_selection = 0

                    else:
                        user_history_selection = int(user_history_selection)
                        user_history_selection -= 1
                        History_Product = clas.Product(HistoryList.history_list
                                                       [user_history_selection][3])
                        ...
                        while run_loop_final_history == 0:
                            ...
                            try:
                                user_back_menu = input("\
Appuyez sur la touche [Entrée] pour retourner au menu principal ")
                                assert user_back_menu == "" \
                                    or user_back_menu == "quit"
        ...
        cursor.execute("""SELECT category_id, category_correct_name
                          FROM Categories""")
        ...
        func.display_categories_in_order()
...
Tapez le code catégorie associé à votre choix et appuyez sur Entrée ==>  ")
        ...
        # We extract the products with selected category
        sql_product_by_category_query = """SELECT id_product, product_name_fr,
                                                  brands, quantity
                                           FROM Products
                                           WHERE category_id=%s"""
        ...
        choice = 0
        products_list = []
        # Loop to create a list with product elements to be displayed on screen
        while choice < len(response):
            sub_choice = response[choice]
            itera = choice + 1
            ...
            products_list.append([itera, name_prod, code_prod,
                                  brands_prod, quantity_prod])
            choice = choice + 1
        ...
        func.display_products_per_page(products_list,
        ...
        while run_loop_product_selection == 0:
            user_choice = input("\
Tapez le nombre ou la lettre associés à votre choix et appuyez sur Entrée \
==>  ")
            # p for previous page
            if user_choice == "p":
            ...
            # s for next page
                ...
                try:
                    ...
                    print("\
Veuillez rentrer le chiffre correspondant ou la lettre de changement de page")
                    run_loop_product_selection = 0

                except AssertionError:
                    print("\
Veuillez rentrer un chiffre présent dans cette page de produits")
                    run_loop_product_selection = 0
        ...
        ChosenProduct = clas.Product(result[2])
        ChosenProduct.show_detailed_product()

        run_loop_replacement = 0
        # Loop to ask the user if she/he wants a replacement
        while run_loop_replacement == 0:
            # If category is orphan, replace by product of same category
            if ChosenProduct.category_id % 10 == 9:
                user_replacement_validation = input("\
Le programme va maintenant rechercher un meilleur produit, appuyez sur la \
touche [Entrée] pour valider")
                if user_replacement_validation == "":
                    ...
                    # Else we create a new class product agent to be displayed
                        ...
                        ReplacementProduct = clas.Product(ChosenProduct.replacement_id_product)
                    ...
                    run_loop_replacement = 0
            # Selected category has a parent
            # the user can choose a replacement in
            else:
                user_replacement_choice = input("\
Souhaitez-vous un remplacement dans la même sous-catégorie ou parmi la \
catégorie générale ? \n\nAppuyez sur [m] pour même categorie ou [g] pour \
catégorie générale => ")
        ...
        while run_loop_history_save == 0:
            user_history_choice = input("\
Souhaitez-vous sauvegarder le résultat de cette recherche ?\n\nAppuyez sur [s] pour \
sauvegarder ou [m] pour retourner au menu principal sans sauvegarder => ")
            # User chooses to save the results
            if user_history_choice == "s":
                sql_insert_history_query = """INSERT INTO History (id_product_research,
                                                                   id_product_results)
                                              VALUES (%s, %s)"""
                tuple_history_insert = (ChosenProduct.id_product,
                                        ChosenProduct.replacement_id_product)
