...
class Product:
    ...
    def __init__(self, id_product):
        ...
        from the product table to easily access by this method
        ...
        sql_query_for_product = """SELECT id_product, code, product_name_fr,
                                          brands, category_id, nutrition_grade,
                                          stores, unique_scan_n, quantity
                                   FROM Products WHERE id_product=%s"""
    ...
    def show_detailed_product(self):
        ...
        # Attempt for a visible rendering of information
    ...
    def find_better_product_same_category(self):
        """Finding the product with best nutritive score within same category.
        ...
        if self.nutrition_grade == "a":
            print("\
Ce produit possède dejà la meilleure note nutritive attribuable")
            # Variable used to create the History table
            self.replacement_id_product = self.id_product

        else:

            # This list allows us to transform alphabetical grades 
            # in numerical grades thanks to their position in the list
            ...
            while targeted_product_grade_index <= selected_product_grade_index:
                ...
                sql_query_for_replacement = """SELECT id_product, code, product_name_fr,
                                                      brands, category_id, nutrition_grade, stores
                                               FROM `Products`
                                               WHERE category_id = %s AND nutrition_grade = %s"""
                ...
                if replacement is None:
                    ...
                    if targeted_product_grade_index == selected_product_grade_index:
                        print("\
Il n'y a pas de produit avec une meilleure note dans cette catégorie")
                        self.replacement_id_product = self.id_product
                    else:
                        print("Le meilleur produit trouvé est le suivant :",
                              replacement.get("product_name_fr"),
                              "avec la note nutritive",
                              replacement.get("nutrition_grade"))
                        # This line to ensure it doesn't check inferior grades,
                        # loop stops at the best product available
        ...
        if self.nutrition_grade == "a":
            ...
            while targeted_product_grade_index <= selected_product_grade_index:
                ...
                sql_query_for_replacement = """SELECT id_product, code, product_name_fr,
                                                      brands, category_id, nutrition_grade, stores
                                               FROM `Products`
                                               WHERE category_id > %s AND category_id < %s
                                                                      AND nutrition_grade = %s"""
                cursor.execute(sql_query_for_replacement, tuple_for_query)
                replacement = cursor.fetchone()
                ...
                if replacement is None:
                    ...
                    if targeted_product_grade_index == selected_product_grade_index:
                        print("\
Il n'y a pas de produit avec une meilleure note dans ces catégories")
                        self.replacement_id_product = self.id_product
                    else:
                        print("\nLe meilleur produit trouvé est le suivant :",
                              replacement.get("product_name_fr"),
                              "avec la note nutritive",
                              replacement.get("nutrition_grade"))
...
class History:
    ...
    def __init__(self):
        ...
        # Query to get the history table with the correct name of the products 
        # from the products table
        cursor.execute("""
            SELECT History.id_research, Ps.product_name_fr AS search,
                   Pr.product_name_fr AS result,
                   History.id_product_results AS id_product
            FROM History
        ...
        while id_selection < len(history):
            ...
            # itera is the number displayed on the left,
            # indication of which number the user has to press
    ...
    def show_history(self):
        ...
        while i < len(self.history_list):
            print(self.history_list[i][0], "<=>", self.history_list[i][1],
                  "remplacé par", self.history_list[i][2])
            i += 1
